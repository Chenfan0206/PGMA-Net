...
class AverageMeter:
    ...
    def __init__(self, dataset):
        ...
        self.class_ids_interest = torch.tensor(
            self.class_ids_interest, device=torch.device('cuda')
        )
        ...
        if self.benchmark == 'pascal':
            ...
            self.nclass = 1000
        elif self.benchmark == 'pascal_extend':
            self.nclass = 20
        elif self.benchmark == 'coco_extend':
            self.nclass = 80
        elif self.benchmark == 'fss_extend':
            self.nclass = 1000
        else:
            raise ValueError('Unknown benchmark: {}'.format(self.benchmark))
        ...
        self.intersection_buf = torch.zeros(
            [2, self.nclass], dtype=torch.float, device=torch.device('cuda')
        )
        self.union_buf = torch.zeros_like(
            self.intersection_buf, dtype=torch.float, device=torch.device('cuda')
        )
...
class Logger:
    ...
    def initialize(cls, args, training):
        ...
        prefix = '_TEST_' if not training else '_TRAIN_'
        if training:
            logpath = '{}/{}/{}/{}'.format(prefix, args.logpath, args.fold, logtime)
            cls.logpath = os.path.join('logs', logpath + '.log')

        else:
            logpath = '{}/nshot_{}/{}'.format(args.load.replace('.pt',''),args.nshot, logtime)
            cls.logpath = logpath

        cls.benchmark = args.benchmark
        os.makedirs(cls.logpath,exist_ok=True)
        print('Log path: %s' % cls.logpath)
    ...
    def save_model_miou(cls, model, epoch, val_miou):
        torch.save(model.state_dict(), os.path.join(cls.logpath, 'epoch_{}_miou_{}.pt'.format(epoch, val_miou)))
    ...
    @classmethod
    def log_frozen_trainable_params(cls, model):
        frozen_param = 0
        trainable_param = 0
        for k in model.state_dict().keys():
            n_param = model.state_dict()[k].view(-1).size(0)
            if k.requires_grad:
                trainable_param += n_param
            else:
                frozen_param += n_param
            # if k.split('.')[0] in 'backbone':
            #     if k.split('.')[1] in [
            #         'classifier',
            #         'fc',
        ...
        Logger.info('Frozen # param.: %d' % frozen_param)
        Logger.info('Trainable # param.: %d' % trainable_param)
        Logger.info('Total # param.: %d' % (frozen_param + trainable_param))
