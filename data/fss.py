...
class DatasetFSS(Dataset):
    ...
    def load_frame(self, query_name, support_names):
        ...
        query_img = cv2.cvtColor(cv2.imread(query_name), cv2.COLOR_BGR2RGB)
...
if __name__=="__main__":
    dataset = DatasetFSS(datapath='Datasets_HSN', fold=0, transform=None,
                         split='test', shot=1, use_original_imgsize=False)
    print(len(dataset))
    for i in range(100):
        batch = dataset[i]
        print(batch['query_img'].shape, batch['query_mask'].shape, batch['query_name'])
        print(batch['support_imgs'].shape, batch['support_masks'].shape, batch['support_names'])
